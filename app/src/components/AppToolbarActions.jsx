 ...
 * Used by both DashboardLayout (user) and AdminLayout (admin).
 *
 * @param {boolean} isAdmin   — show "Admin Panel" link in profile dropdown
 * @param {boolean} inAdmin   — currently inside admin area (show "กลับหน้าผู้ใช้" instead)
 */
export default function AppToolbarActions({ inAdmin = false }) {
  const { enqueueSnackbar } = useSnackbar();
  const { mode, toggleTheme } = useThemeMode();
  const theme = useTheme();
  const navigate = useNavigate();
  const white = theme.palette.common.white;
  const userInfoRef = React.useRef(getUserInfo());
  ...
  const handleMarkAllRead = async () => {
    if (!userInfo?.user_id) return;
    try {
      await apiClient.put(`/api/notifications/user/${userInfo.user_id}/read-all`);
      setUnreadCount(0);
  ...
  const handleLogout = () => {
    ...
    enqueueSnackbar("ออกจากระบบเรียบร้อย", { variant: "info", autoHideDuration: 2000 });
    SysSignout();
  };

  const iconBtnSx = {
    color: alpha(white, 0.85),
    "&:hover": { color: white, bgcolor: alpha(white, 0.12) },
  };
  ...
  return (
    <Stack direction="row" spacing={0.5} alignItems="center">
      ...
      <Tooltip title="แจ้งเตือน">
        <IconButton size="small" onClick={(e) => { setNotifAnchor(e.currentTarget); fetchNotifications(); }} sx={iconBtnSx}>
      ...
      <Popover
        ...
        slotProps={{ paper: { sx: { width: 360, maxHeight: 480, mt: 1, borderRadius: 2, overflow: "hidden" } } }}
      >
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 2, py: 1.5 }}>
          <Typography variant="subtitle1" fontWeight={700}>แจ้งเตือน</Typography>
          {unreadCount > 0 && (
            <Button size="small" startIcon={<DoneAllIcon sx={{ fontSize: 16 }} />} onClick={handleMarkAllRead} sx={{ textTransform: "none", fontSize: "0.75rem" }}>
              อ่านทั้งหมด
            </Button>
          )}
        </Stack>
        ...
        <List disablePadding sx={{ maxHeight: 340, overflowY: "auto" }}>
          {notifications.length === 0 ? (
            <ListItem>
              <Typography variant="body2" color="text.secondary" sx={{ py: 3, textAlign: "center", width: "100%" }}>
                ไม่มีแจ้งเตือน
              </Typography>
            </ListItem>
          ) : notifications.map((n) => (
            ...
            >
              <Stack spacing={0.5} sx={{ width: "100%" }}>
                <Stack direction="row" alignItems="center" spacing={1}>
                  {!n.is_read && <CircleIcon sx={{ fontSize: 8, color: "primary.main", flexShrink: 0 }} />}
                  <Typography variant="body2" fontWeight={n.is_read ? 400 : 600} noWrap sx={{ flex: 1 }}>{n.title}</Typography>
                  <Chip label={n.severity || "info"} size="small" color={SEVERITY_COLORS[n.severity] || "default"} sx={{ height: 18, fontSize: "0.65rem" }} />
                </Stack>
        ...
        <Box sx={{ p: 1, textAlign: "center" }}>
          <Button size="small" fullWidth onClick={() => { setNotifAnchor(null); navigate(ROUTES.NOTIFICATIONS); }} sx={{ textTransform: "none", fontSize: "0.8rem" }}>
            ดูแจ้งเตือนทั้งหมด
          </Button>
        </Box>
      </Popover>
      ...
      <Chip
        avatar={
          <Avatar sx={{ bgcolor: alpha(white, 0.2), color: white, fontSize: 12, fontWeight: 700 }}>
        ...
        sx={{
          color: white, borderColor: alpha(white, 0.3), cursor: "pointer",
          "& .MuiChip-label": { fontWeight: 500, fontSize: "0.8rem" },
          "&:hover": { borderColor: alpha(white, 0.6), bgcolor: alpha(white, 0.08) },
        }}
      />
      ...
      >
        ...
        {/* Admin Panel link — only for admin, only in user area */}
        {isAdmin && !inAdmin && (
        ...
        <MenuItem onClick={() => { setProfileAnchor(null); navigate(ROUTES.SETTINGS); }}>
          <ListItemIcon><SettingsIcon fontSize="small" /></ListItemIcon>
          <ListItemText>ตั้งค่า</ListItemText>
        </MenuItem>
        <MenuItem onClick={() => { setProfileAnchor(null); toggleTheme(); }}>
          <ListItemIcon>{mode === "light" ? <DarkModeIcon fontSize="small" /> : <LightModeIcon fontSize="small" />}</ListItemIcon>
          <ListItemText>{mode === "light" ? "โหมดมืด" : "โหมดสว่าง"}</ListItemText>
        </MenuItem>
        <Divider />
